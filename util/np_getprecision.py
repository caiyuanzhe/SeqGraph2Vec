...
def precision(
    ...
    path_to_subseg_name='../data_dir/output/seg2vec/SubSegmentNames-75bp.txt',
...
def my_load_txt(file):
    ret = list()
    with open(file, 'r') as f:
        for line in f:
            ret.append(np.array(line.split('\n')[0]))
    ret = np.array(ret)
...
# 自定义距离函数
def my_distance(x, y):
    # 计算查询向量和数据集中某个向量之间的距离
    relu_dot_product =  np.maximum(np.dot(x, y), 0)
    non_zero_indices = np.nonzero(relu_dot_product)[0]

    x_sum_of_squares = np.sum(x[non_zero_indices] ** 2)
    x_l2_norm = np.sqrt(x_sum_of_squares)

    y_sum_of_squares = np.sum(y[non_zero_indices] ** 2)
    y_l2_norm = np.sqrt(y_sum_of_squares)

    dist = relu_dot_product / (x_l2_norm * y_l2_norm)
    return dist
...
k = 5
nn = NearestNeighbors(n_neighbors=k, algorithm='brute', metric=my_distance)

# 将数据集添加到 NearestNeighbors 对象中
nn.fit(x)

# 搜索
distances, indices = nn.kneighbors([q1,q2])
print(distances)   # 输出距离数组
print(indices)     # 输出索引数组



""" Formal Test """
# work_dir = '../data_dir/input/retrieval_for_fungi/'
work_dir = './tmp/'

segment_name_file = work_dir + 'segment.txt',
segment_vec_file = work_dir + 'SegmentVectors.txt'

subsegment_name_file = work_dir + 'extracted_org_segment.txt',
subsegment_vec_file = work_dir + 'SubSegmentVectors.txt'


 # segment vectors as key
xk = np.loadtxt(segment_vec_file)
xk = xk.astype(np.float32)

 # sub-segment vectors as query
xq = np.loadtxt(subsegment_vec_file)
xq = xq.astype(np.float32)


# compare two groups of DNA segments
print(subsegment_name_file)
obj1 = np.loadtxt(subsegment_name_file[0], dtype=np.str_)
obj2 = np.loadtxt(segment_name_file[0], dtype=np.str_)
print(f"There are {len(obj2)} vectors")
...
# 搜索
distances, indices = nn.kneighbors(xq)
# print(distances)   # 输出距离数组
# print(indices)     # 输出索引数组
