...
@njit(nogil=True)
def extract_kmer_variable_overlap(seq: str, overlap: int, mer: int):
    """ Return a DNA sequence's k-mers. Slide nucleotides of 'overlap' length
    E.g.,
    ...
    for i in range(0, 3):
        ...
        tmp_kmers = []
        j = 0
        while j <= (len(tmp_seq) - mer):
            tmp_kmers.append(tmp_seq[j:j+mer])
            j += (mer - overlap)
        seq_corpus.append(tmp_kmers)
    # print(seq_corpus)
    return seq_corpus


#extract_kmer_variable_overlap('ACGATGAA', 2, 3)

@njit(nogil=True)
def seq2sentence(seqs: List[str], overlap: int = 5, mer: int = 8):
    """ Express a sequence in NLP sentence style.
    Note:
    ...
    for seq in seqs:
        tmp = list()
        for group in extract_kmer_variable_overlap(seq, overlap, mer):
            sentence = ' '.join(group)
            tmp.append(sentence)
...
#seq2sentence(['ACGATGAAACGATGAAACGATGAA', 'ACGATGAACGATGAA'])
...
@njit(nogil=True)
def multiscale_seg2sentence(segs: List[str], mers: List = [6, 7, 8]):
    """ Express a segment in NLP sentence style.
    Note:
        ['segments'] --> ['seg egm gme men ent nts']
    """
    ...
    for seg in segs:
        tmp = list()
        for mer in mers:
            tmp.append(' '.join(extract_kmer(seg, mer)))
        sentences.append(tmp)
    return sentences
