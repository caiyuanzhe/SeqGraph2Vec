...
from src.sparse_rw import SparseRWGraph
        ...
        def node2vec_walks(num_iter, progress_proxy):
            ...
            for i in prange(num_iter):
                ...
                if has_nbrs(start_node_idx):
                    ...
                    walk_idx_mat[i, 1] = random.randint(0, num_nodes-1)
                    # continue
                ...
                for j in range(2, walk_length + 1):
                    ...
                    if has_nbrs(cur_idx):
                        ...
                        if prob < alpha:  
                            walk_idx_mat[i, j] = move_forward(cur_idx, prev_idx)  # normally transfer to a neighboring node at probability (alpha)
                        else:
                            walk_idx_mat[i, j] = random.randint(0, num_nodes-1)  # randomly teleport to a node at probability (1-alpha)
                    else:  # sink node
                        walk_idx_mat[i, j] = random.randint(0, num_nodes-1)  # randomly teleport to a node
